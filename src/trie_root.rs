...
pub fn hex_prefix_encode<'a>(nibbles: &'a [u8], leaf: bool) -> impl Iterator<Item = u8> + 'a {
	...
	let first_byte = {
		// is inlen odd or even: 1 for odd, 0 for even
		// 00: even & extension
		// 10: odd & exntension
		// 20: even & leaf
		// 30: odd & leaf
	...
	// turn each element from nibbles from nibble into byte
	// by combining every 2 of them
		...
		H: Hasher,
{
	...
	// take slices
	// take the first path and node
	// which is nibbles(rlp(index)) and rlp(tx)
	...
	if inlen == 1 {
		stream.begin_list(2);
		// after `hex_prefix_encode`, each element is a byte
